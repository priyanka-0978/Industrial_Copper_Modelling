...
with tab1:
    ...
    with st.form("my_form"):
        ...
        if submit_button and flag == 1:
            if len(i) == 0:
                st.write(':red[Please enter a valid number, spaces are not allowed between the value]')
            else:
...
with tab2: 
        ...
        if csubmit_button and cflag==1:
            if len(k)==0:
                st.write(":red[Please enter a valid number space not allowed]")
            else:
                st.write(":red[You have entered an invalid value:]",k)  
